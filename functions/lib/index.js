...
exports.serveListingDetail = exports.verifyRecaptcha = void 0;
 ...
 */
exports.verifyRecaptcha = (0, https_1.onCall)({ secrets: [recaptchaSecret], region: "europe-west1" }, async (request) => {
    const token = request.data?.token;
    if (!token || typeof token !== "string") {
        throw new https_1.HttpsError("invalid-argument", "reCAPTCHA-Token fehlt oder ist ungültig.");
    }
    const secretKey = recaptchaSecret.value();
    if (!secretKey) {
        console.error("RECAPTCHA_SECRET_KEY is not configured.");
        throw new https_1.HttpsError("internal", "reCAPTCHA-Konfiguration fehlt auf dem Server.");
    }
    ...
    try {
        ...
        if (!result.success) {
            ...
            throw new https_1.HttpsError("permission-denied", "reCAPTCHA-Verifizierung fehlgeschlagen. Bitte versuche es erneut.");
        }
        return { success: true };
    }
    catch (error) {
        ...
        throw new https_1.HttpsError("internal", "Fehler bei der reCAPTCHA-Verifizierung.");
    }
});
/**
 ...
 */
exports.serveListingDetail = (0, https_1.onRequest)({ region: "europe-west1" }, async (req, res) => {
